        ...
        unsigned int out_streams, boost::shared_ptr<gr::msg_queue> queue)
{
    ...
    else if (implementation.compare("GPS_L1_CA_DLL_KF_Tracking") == 0)
            {
                std::unique_ptr<GNSSBlockInterface> block_(new GpsL1CaDllKfTracking(configuration.get(), role, in_streams,
                        out_streams));
                block = std::move(block_);
            }
    ...
    if (implementation.compare("GPS_L1_CA_DLL_PLL_Tracking") == 0)
        {
            ...
            {
                std::unique_ptr<TrackingInterface> block_(new GpsL1CaDllKfTracking(configuration.get(), role, in_streams,
                        out_streams));
                block = std::move(block_);
            }
    else if (implementation.compare("GPS_L1_CA_DLL_PLL_C_Aid_Tracking") == 0)
